  ...
  Future<List<Video>> fetchVideos() async {
    ...
    try {
      ...
      final getTimeline = await bluesky.feed.getTimeline(
        algorithm: 'reverse-chronological', // algoritmo para ordenação (mais recentes primeiro)
        limit: _limit,
        cursor: _cursor,
      );
      ...
      final videoItems = getTimeline.data.feed.where((item) {
        ...
        if (embedType?.contains('media') ?? false) {
          ...
          if (mediaItems != null && mediaItems.isNotEmpty) {
            return mediaItems.any((media) => 
              (media['mimetype'] as String?)?.startsWith('video/') ?? false);
          }
        }
      ...
      return videoItems.map((item) => Video.fromJson({
        'post': item.post.toJson(),
      })).toList();
